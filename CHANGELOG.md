 ...
 * Requests can now be grouped with `tag` and can be batch paused, resumed, or canceled.
 * Resizing with either height or width of 0 will now maintain aspect ratio.
 * Exposed a setter for global Picasso instance.
 * `stableKey` provides an override value for the URI or resource ID when caching.
